...
class minimizar (object):
	...
	def reIstanciate(self):
		...
		mostrar = self.dfa_minimizado.afd
		for x in mostrar:
			list(x)
			y = tuple(self.estado_inicialAFD)
			if  len(x[0]) > 9:
				self.transi.append(y[0])
				self.transi.append(x)
		
		prueba = self.transi
		#Guardar en un txt
		
		nombre_archivo = input('\nIngrese el nombre del archivo para guardar el AFD minimizado -> ')
		...
		if os.path.exists(nombre_archivo):
			...
			with open(nombre_archivo, 'a', encoding="utf-8") as f:
				f.write("AFD Minimizado -->")
				f.write("\n")
				f.write("Símbolos: "+ str(self.simbolosAFD))
				f.write("\n")
				f.write("Estados:  " + str(nuevos_estados))
				f.write("\n")
				f.write("Estado inicial: { " + str(self.estado_inicialAFD) + " }")
				f.write("\n")
				f.write("Estados de aceptación: { " + str(estados_finalesAFD) + " }")
				f.write("\n")
				f.write("Transiciones: " + str(prueba))
